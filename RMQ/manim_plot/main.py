    ...
    def construct(self):
        ...
        for i in range(4):
            for j in  range(i, 4):
                ...
                self.play(Create(self.plot_rect(j - i + 1, array1[i].get_center(), RED), run_time=0.4),
                          Create(self.plot_rect(j - i + 1, array2[i].get_center(), RED), run_time=0.4))
                self.wait(0.3)
...
class Fig2(Scene):
    def construct(self):
        ...
        for i in range(len(a)):
            ...
            self.play(ReplacementTransform(v[i].copy(), vb[i], run_time=0.4))
        ...
        self.wait(0.3)
        self.play(a[0].animate.move_to(LEFT * 6 + UP * 3), run_time=0.4)
        self.play(a[3].animate.next_to(a[0], RIGHT), run_time=0.4)

        self.play(a[1].animate.next_to(a[0], DOWN * 3), run_time=0.4)
        self.play(a[7].animate.next_to(a[1], RIGHT), run_time=0.4)

        self.play(a[5].animate.next_to(a[1], DOWN * 3), run_time=0.4)
        self.play(a[6].animate.next_to(a[5], RIGHT), run_time=0.4)

        self.play(a[2].animate.next_to(a[5], DOWN * 3), run_time=0.4)

        self.play(a[4].animate.next_to(a[2], DOWN * 3), run_time=0.4)
...
class Fig5(Scene):
    def construct(self):
        ...
        self.play(Create(self.plot_rect(7, a[0].get_center(), RED_E), run_time=0.4))
        self.wait(0.2)
    ...
    def sub_plot(self, l, r, x, a, u, v):
        self.play(Create(self.plot_rect(r - l + 1, a[l].get_center(), RED_E), run_time=0.4))
        self.wait(0.3)
...
class Fig6(Scene):
    def construct(self):
        ...
        self.play(Create(self.plot_rect(5, a[0].get_center(), RED_E), run_time=0.4))
        self.wait(0.3)
        ...
        for i in range(len(vv)):
            ...
            while len(stack) > 0 and vv[stack[-1].index] > vv[i]:
                ...
                if len(stack) <= 0 or vv[stack[-1].index] <= vv[i]:
                    break
                self.play(FadeOut(stk.pop(), shift=RIGHT, run_time=0.4))
            self.wait(0.2)
            if last_poped_node != None:
                self.play(Transform(stk.pop(), last_poped_node, run_time=0.4))
                if new_node.l_line:
                    new_node.add_l_child(last_poped_node)
                    line = Line(new_node, last_poped_node, color=BLACK)
                    self.play(ReplacementTransform(new_node.l_line, line, run_time=0.4))
            ...
            if len(stack) > 0:
                ...
                if stack[-1].r_line:
                    ...
                    self.play(ReplacementTransform(stack[-1].r_line, line, run_time=0.4))
                    stack[-1].r_line = line
                else:
                    stack[-1].add_r_child(new_node)
                    line = Line(stack[-1], new_node, color=BLACK)
